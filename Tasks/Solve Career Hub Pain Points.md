---
status: done
priority: high
scheduled: 2026-06-29
dateCreated: 2026-06-29T11:07:51.906+01:00
dateModified: 2026-06-29T11:25:07.286+01:00
tags:
  - task
contexts:
  - hub
projects:
  - "[[Career Hub Management]]"
timeEntries:
  - startTime: 2026-06-29T10:10:58.453Z
    description: Work session
    endTime: 2026-06-29T10:25:07.286Z
completedDate: 2026-06-29
tasknotes_manual_order: tndsowhldsow
---

- [x]  I have to manual start a operation note every day by making a copy of the old operation note under the folder, then change the date and delete the previous notes... etc, this will easily caused mistakes such as wrong folder or wrong date;
 - [x] Its not very friendly when i try to look for on going task or projects, I have to scroll through the sidebar to look for the relevant folder;
 - [x] there is not a home page that can gives an overview of what's going on to help user picking up work easily;
