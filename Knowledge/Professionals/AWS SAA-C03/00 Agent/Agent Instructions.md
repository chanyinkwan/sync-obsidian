...
- **Read-only context:** `Template/SAA *.md` (the four templates), `Projects/AWS SAA-C03.md` (working loop), `Projects/SA Presales Transition.md` (capacity and gates), `00 Agent/Study Triage.md` (per-topic target state), `00 Agent/Cantrill Index.md` (syllabus-id → lecture → video URL).
...
1. **Atomic notes** → `02 Notes/`, using SAA Knowledge Note Template. Title is a one-sentence claim, never a topic. One idea per note. Status walks capture → distilled → connected → mastered. The title-is-a-claim rule applies to Knowledge Notes; Question Notes, Study Sessions, and Content Ideas may use descriptive titles. At creation, assign `topics:` (one or more ids from `01 Dashboard/Syllabus.md`) in frontmatter; a note with no valid topic id renders in the readiness dashboard's unmapped pile.
2. **Question log** → `03 Questions/`, using SAA Question Note Template. Only wrong / guessed / slow questions. Paraphrase the gist; never paste a full question. Every question note must link to (or spawn) exactly one knowledge note, or be flagged for archive. At creation, assign the same `topics:` id(s) as its linked knowledge note. After **every** mock, autopsy each wrong / guessed / slow question into a question note and tag it exactly one of `knowledge-gap` | `misread` | `trap-pattern`.
3. **Learning journey** → `04 Journey/`, using SAA Study Session Template. One note per session, filename `YYYY-MM-DD Topic`. Exactly one next-action per session. Also capture any Cantrill JIT remediation video assigned during the session (name the exact lecture, not "review this domain") as a flag on the relevant note; resolve lecture references against [[Cantrill Index]].
...
- **Topic state ladder (canonical).** This is the single definition of the state model; the readiness view, [[Exam Map]], and [[Skill - Run Study Session]] all reference it rather than restating it. State is agent-determined from session content and quiz performance, never self-reported. Every syllabus topic sits at one rung:
  - **Untouched → Touched:** the topic was engaged with in a live session, so a knowledge or question note now maps to it.
  - **Touched → Proven:** the four-part mastery test below is satisfied. **Proven and "mastered" are the same rung**; the mastery test is the definition of Proven, not a competing one.
  A Proven topic knocked back by a later wrong or guessed question becomes **contested**. Per-topic *targets* (which rung is "done enough") live in [[Study Triage]], not here. A JIT remediation video assigned mid-session (see the escalation ladder in [[Study Session Protocol]]) is recorded as a **flag** on the relevant note, never as a state.
- **Proven needs evidence (the four-part mastery test).** A knowledge note may be marked mastered ( = Proven) only when ALL four hold, with the evidence cited in the note:
...
- **Respect each topic's target state.** Do not chase Proven on every topic. [[Study Triage]] assigns each syllabus-id a target: **Core → Proven**, **Tail → Functional** (recognise and discriminate; Touched is acceptable, Proven not required), **Sacrifice → skip** before the exam. When triaging unknowns and reporting coverage, measure a topic against ITS target, and never spend mastery effort pushing a Tail or Sacrifice topic past its target.
