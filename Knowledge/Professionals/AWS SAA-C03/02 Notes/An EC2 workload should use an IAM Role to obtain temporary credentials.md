...
## 論點(用自己的話,3-5 句)
- An application on an EC2 instance assumes an IAM Role and receives rotating temporary credentials. It signs AWS API requests with those credentials, allowing AWS to resolve the Role principal and evaluate permissions. The Role controls permitted Actions, Resources, and Conditions, while the EC2 instance type controls CPU and memory. Do not hard-code long-term access keys or use root credentials. The application itself never authorises the call; it only signs the request. Authorisation is decided on AWS's side by the IAM policy evaluation engine after the signed request arrives, not inside the application layer.

## 考點 / Trap(exam 會怎樣考你)
- Prefer an attached least-privilege IAM Role over IAM User access keys stored in application code.
- The real reason hardcoded keys are wrong is not "harder to update": leaked long-term keys stay valid until someone manually revokes them, while temporary credentials expire on their own.
...
## 連結
- 來源 Question [[Choose an IAM Role when an EC2 application needs S3 access]], [[Authorization happens on AWS's side, not inside the requesting application]]
