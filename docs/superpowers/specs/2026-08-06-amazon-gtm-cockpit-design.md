...
`useBodyTemplate` is currently `false`. Turn it on with this `bodyTemplate`:
...
## Stakeholder read
| Who | Decision right | Their incentive | Likely stance | What gets a yes |
|-----|----------------|-----------------|---------------|-----------------|
...
**Durable** incentives are written once into `Relationship Management/` person notes, as new frontmatter:
...
The content for the core Amazon cast is already written in Amazon Operations Glossary §1 and needs only to be moved into the person notes:

- 各國子網 — have their own revenue KPI, so they want lower prices to sell now; the standing conflict is that Kess's priority is not going out of stock while theirs is hitting this month's revenue.
- [[yubeifei y00663235|俞碧斐]] — holds no resources directly but can go and get them (MOQ reductions, marketing budget). Supply allocation is his real lever.
- [[Li Qinghua 00861267|李清華]] — the approval gate for on-top price requests. Breaking the 大促 floor requires her or 碧斐 first.
- [[Zhang Xuan 00942107|張炫]] — 地區部 GTM for router and 齊軒's manager. All router matters go to him.
- Eric — decision on Amazon-side delivery. Tony — execution.

**Situational** stance lives in the task body. The durable half is never retyped.
...
1. **Plan B has an unowned critical path.** It requires Selina's and 程哥's international departure **city and date** before 蒙清萍 can ship anything. Nobody in the plan table owns obtaining that. If it arrives on 8.15 the plan is dead. This is Kess's to chase, this week.
...
3. **Plan A and Plan B are being evaluated by two people who are not talking.** 张程 on air freight, 刘宙 on carry-on. The coordination risk is one of them idling while waiting on the other's answer.

### Stakeholder read for this task
...
| 5 | Create the E6898 parent and three children as the first real load; create the three missing person notes | 20 min |

Roughly two and a quarter hours.
